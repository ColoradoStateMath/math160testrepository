\documentclass[handout]{ximera}
%\documentclass[10pt,handout,twocolumn,twoside,wordchoicegiven]{xercises}
%\documentclass[10pt,handout,twocolumn,twoside,wordchoicegiven]{xourse}

%\author{Steven Gubkin}
%\license{Creative Commons 3.0 By-NC}
\input{../preamble.tex}

\outcome{Exercises: Second Derivative Test}
   

\title{Second Derivative Test Exercises}

\begin{document}
\begin{abstract}
  Here we'll practice using the second derivative test.
\end{abstract}
\maketitle

%%PROBLEM 1
\begin{exercise}
The function $f(x) = x^3-27x+6$ has two critical points.  If we call
these critical points $a$ and $b$, and order them such that $a < b$,
then

$$
a = \answer{-3}
$$

$$
b=\answer{3}
$$

$f''(a)$ is \wordChoice{\choice{positive} \choice[correct]{negative}}, so $f(a)$ is a local \wordChoice{\choice[correct]{max} \choice{min}}.

$f''(b)$ is \wordChoice{\choice[correct]{positive} \choice{negative}}, so $f(b)$ is a local \wordChoice{\choice{max} \choice[correct]{min}}.
\end{exercise}

%%PROBLEM 2
\begin{exercise}
The function $f(x) = \displaystyle\frac{x}{1+x^2}$ has two critical points.  If we call
these critical points $a$ and $b$, and order them such that $a < b$,
then

$$
a = \answer{-1}
$$

$$
b=\answer{1}
$$

$f''(a)$ is \wordChoice{\choice[correct]{positive} \choice{negative}}, so $f(a)$ is a local \wordChoice{\choice{max} \choice[correct]{min}}.

$f''(b)$ is \wordChoice{\choice{positive} \choice[correct]{negative}}, so $f(b)$ is a local \wordChoice{\choice[correct]{max} \choice{min}}.
\end{exercise}

%%PROBLEM 3
\begin{exercise}
Consider the function $f(x) = x\sqrt{4-x}$.

What is the domain of $f(x)$?  Domain: $(-\infty,  \answer{4}]$

$f(x)$ has only one critical point, $x=a$.

$$
a = \answer{8/3}
$$


$f''(a)$ is \wordChoice{\choice{positive} \choice[correct]{negative}}, so $f(a)$ is a local \wordChoice{\choice[correct]{max} \choice{min}}.
\end{exercise}

%%PROBLEM 4
\begin{exercise}
The function $f(x) = x^4-4x^3+16x-3$ has two critical points. If we
call these critical points $a$ and $b$, and order them such that $a <
b$, then

$$
a = \answer{-1}
$$

$$
b=\answer{2}
$$

At $x=a$, the second derivative test
\begin{multipleChoice}
\choice{Indicates a local maximum occurs at $x=a$}
\choice[correct]{Indicates a local minimum occurs at $x=a$}
\choice{Fails, but the First derivative test indicates $x=a$ is the location of a local max}
\choice{Fails, but the First derivative test indicates $x=a$ is the location of a local min}
\choice{Fails, and the First derivative test indicates that $x=a$ is not the location of a local extrema}
\end{multipleChoice}

At $x=b$, the second derivative test
\begin{multipleChoice}
\choice{Indicates a local maximum occurs at $x=b$}
\choice{Indicates a local minimum occurs at $x=b$}
\choice{Fails, but the First derivative test indicates $x=b$ is the location of a local max}
\choice{Fails, but the First derivative test indicates $x=b$ is the location of a local min}
\choice[correct]{Fails, and the First derivative test indicates that $x=b$ is not the location of a local extrema}
\end{multipleChoice}

\end{exercise}

%%PROBLEM 5
\begin{exercise}
The function $f(x) = x^4-4 x^3+6 x^2-4 x+3$ has only one critical point, $x=a$.

$$
a = \answer{1}
$$

At $x=a$, the second derivative test
\begin{multipleChoice}
\choice{Indicates a local maximum occurs at $x=a$}
\choice[correct]{Indicates a local minimum occurs at $x=a$}
\choice{Fails, but the First derivative test indicates $x=a$ is the location of a local max}
\choice[correct]{Fails, but the First derivative test indicates $x=a$ is the location of a local min}
\choice{Fails, and the First derivative test indicates that $x=a$ is not the location of a local extrema}
\end{multipleChoice}

\end{exercise}























\end{document}
